...
import {
  followingBlockToggle,
  followingFollowToggle,
  followingMuteToggle,
  followingSendFellowReq,
  followingUpdateConnectionStatus
...
class ComDialog extends Component {
  ...
  onToggleFellow = () => {
    ...
    if (selectedUser.various_status.is_fellow === 'request_accepted') {
      this.props.followingUpdateConnectionStatus(selectedUser.id, 'unfellow');
    } else if (selectedUser.various_status.is_fellow === 'request_sent') {
      this.props.followingUpdateConnectionStatus(selectedUser.id, 'unfellow');
    } else {
      this.props.followingSendFellowReq(selectedUser.id);
    }
    this.hideDialog();
  };
  /**
  ...
  onToggleFollow = () => {
    const {selectedUser} = this.props;
    this.props.followingFollowToggle(selectedUser.id);
    this.hideDialog();
  };
  /**
   * Toggle Mute User
  ...
  onToggleMute = () => {
    const {selectedUser} = this.props;
    this.props.followingMuteToggle(selectedUser.id);
    this.hideDialog();
  };
  /**
   * Toggle block User
  ...
  onToggleBlock = () => {
    const {selectedUser} = this.props;
    this.props.followingBlockToggle(selectedUser.id);
