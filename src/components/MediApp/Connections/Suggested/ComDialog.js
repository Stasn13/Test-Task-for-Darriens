...
import {
  suggestedBlockToggle,
  suggestedFollowToggle,
  suggestedMuteToggle,
  suggestedSendFellowReq,
  suggestedUpdateConnectionStatus
...
class ComDialog extends Component {
  ...
  onToggleFellow = () => {
    ...
    if (selectedUser.various_status.is_fellow === 'request_accepted') {
      this.props.suggestedUpdateConnectionStatus(selectedUser.id, 'unfellow');
    } else if (selectedUser.various_status.is_fellow === 'request_sent') {
      this.props.suggestedUpdateConnectionStatus(selectedUser.id, 'unfellow');
    } else {
      this.props.suggestedSendFellowReq(selectedUser.id);
    }
    this.hideDialog();
  };
  /**
  ...
  onToggleFollow = () => {
    const {selectedUser} = this.props;
    this.props.suggestedFollowToggle(selectedUser.id);
    this.hideDialog();
  };
  /**
   * Toggle Mute User
  ...
  onToggleMute = () => {
    const {selectedUser} = this.props;
    this.props.suggestedMuteToggle(selectedUser.id);
    this.hideDialog();
  };
  /**
   * Toggle block User
  ...
  onToggleBlock = () => {
    const {selectedUser} = this.props;
    this.props.suggestedBlockToggle(selectedUser.id);
    this.hideDialog();
  };
