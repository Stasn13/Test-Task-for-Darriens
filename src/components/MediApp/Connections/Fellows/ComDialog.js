...
import {
  ...
  fellowsFollowToggle,
  fellowsMuteToggle,
  fellowsSendFellowReq,
  fellowsUpdateConnectionStatus
...
class ComDialog extends Component {
  ...
  onToggleFellow = () => {
    ...
    if (selectedUser.various_status.is_fellow === 'request_accepted') {
      this.props.fellowsUpdateConnectionStatus(selectedUser.id, 'unfellow');
    } else if (selectedUser.various_status.is_fellow === 'request_sent') {
      this.props.fellowsUpdateConnectionStatus(selectedUser.id, 'unfellow');
    } else {
      this.props.fellowsSendFellowReq(selectedUser.id);
    }
    this.hideDialog();
  };
  /**
  ...
  onToggleFollow = () => {
    const {selectedUser} = this.props;
    this.props.fellowsFollowToggle(selectedUser.id);
    this.hideDialog();
  };
  /**
   * Toggle Mute User
  ...
  onToggleMute = () => {
    const {selectedUser} = this.props;
    this.props.fellowsMuteToggle(selectedUser.id);
    this.hideDialog();
  };
  /**
   * Toggle block User
  ...
  onToggleBlock = () => {
    const {selectedUser} = this.props;
    this.props.fellowsBlockToggle(selectedUser.id);
