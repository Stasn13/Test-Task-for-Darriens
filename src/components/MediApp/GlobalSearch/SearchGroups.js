...
import {getFilteredGroups, getFilteredUsers} from "../../../actions/GlobalSearchAction";
import GroupItem from "../Groups/GroupList/GroupItem";
...
class SearchGroups extends Component {
  ...
  fetchMore = (page) => {
    this.props.getFilteredGroups({keywords: this.props.searchQuery, page, hideLoading: true});
  };
    ...
    return (
      <View style={styles.sectionContainer}>
        <FlatList
          ...
          renderItem={(row, index) => <GroupItem item={row.item}/>}
          keyExtractor={(item, index) => index.toString()}
          ItemSeparatorComponent={() => <View style={styles.separator}/>}
          onEndReachedThreshold={10}
          onEndReached={() => {
            if (searchQuery && groups.length > 0 && !isLoadingMore && groups.length === this.state.page * 10) {
...
export default connect(null, {getFilteredGroups})(SearchGroups);
