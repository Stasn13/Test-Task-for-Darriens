...
import {getFilteredPages} from "../../../actions/GlobalSearchAction";
...
class SearchPages extends Component {
  ...
  fetchMore = (page) => {
    this.props.getFilteredPages({keywords: this.props.searchQuery, page, hideLoading: true});
  };

  render() {
    const {pages, searchQuery} = this.props;
    ...
    return (
      <View style={styles.sectionContainer}>
        <FlatList
          ...
          onEndReached={() => {
            if (searchQuery && pages.length > 0 && !isLoadingMore && pages.length === this.state.page * 10) {
...
export default connect(null, {getFilteredPages})(SearchPages);
