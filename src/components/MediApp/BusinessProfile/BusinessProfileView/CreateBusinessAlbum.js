...
class CreateBusinessAlbum extends Component {
  constructor(props) {
    super(props);
  ...
  uploadMediaAction(imageObje) {
    ...
    GlobalHelper.uploadAWS3(file).then((response) => {
      if (response.status === 201) {
        ...
        that.props.createBusinessAlbumService(that.props.id, that.state.albumName, that.state.description, s3Response.key, that.props.businessGalleryThis, this.props.isShowFullImage);
      } else {
...
export default connect(mapStateToProps, { createBusinessAlbumService })(CreateBusinessAlbum);
