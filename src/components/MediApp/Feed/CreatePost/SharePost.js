...
import {sharePost, updateLocation, updateTaggedUser} from "../../../../actions/FeedAction";
...
import SharePostItem from "../PostList/PostItem/SharePostItem";
...
class SharePost extends AppComponent {
  ...
  sendToServer = () => {
    const {share_type, post_tag_ids, post_meta, post_location, medias} = this.state;
    ...
    this.props.sharePost({
      shared_post_id: this.props.post.id,
      share_type,
      post_tag_ids,
      post_meta,
      post_location,
      ...
      <KeyboardAvoidingView style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <TouchableOpacity style={{flexDirection: 'column', flex: 1}} onPress={() => Keyboard.dismiss()}>
          ...
          <View style={{flex: 1, flexDirection: 'column', paddingHorizontal: padding.lg, paddingVertical: 15}}>
            ...
            <ScrollView contentContainerStyle={{flexGrow: 1}} style={{height: (Dimensions.get('window').height - 150)}}>
...
const mapStateToProps = ({auth, commonData, feedData}) => {
  const {user} = auth;
